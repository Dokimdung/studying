...
const router = useRouter();
    ...
    return (
        < div className="m-20">
            <h3 className="text-[#252641] text-[30px] p-5 "> Classes tought by real crearors </h3>
            <div className="grid grid-cols-3 gap-3">
                {
                    blogList?.length && blogList.map((blog: any, index: number) => {
                        return (
                            <div onClick={() => router.push('/literature')} key={index + blog.id}
                                 className={" p-3 border-none  rounded-3xl  shadow-[ 0px 0px 20px #262D7614]  text-center "}>
                                <img className='p-7 rounded-[10px]' style={{background: blog?.color}} src={blog?.img} alt=""/>
